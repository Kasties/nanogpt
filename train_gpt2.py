...
def forward(params, idx, is_training=False, target=None, key=None):
    ...
    def feedforward(x, layer_idx):
        out = x @ params['W_ffwd'][layer_idx] # (B, T, n_embd) @ (n_embd, 4*n_embd) -> (B, T, 4*n_embd)
        out = jax.nn.gelu(out,approximate=True) # use approximate gelu as it was the original activation in GPT2
...
# 2. Convert to standard numpy float32
