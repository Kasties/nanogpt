...
def init_model_params(key, vocab_size, n_embd):
    ...
    params = {
        ...
        'layers': {
            ...
            # ============================================================
            # CHANGE 8: Value residual lambda — per-layer blending weight
            #   - Initialized to 0.5: equal mix of current V and layer-0 V
            #   - Learned during training via Muon (simple SGD, no Newton-Schulz)
            # ============================================================
...
def forward(params, idx):
    ...
    # ============================================================
    # CHANGE 8b: Value residual forward pass
    #   - Layer 0 runs separately to capture v1 (first layer's V)
    #   - Layers 1-11 run via scan, blending v1 into their V
    #   - v1 is captured in closure — same v1 for all layers
    #   - Was: single scan over all 12 layers, no value residual
    # ============================================================

    # Split params: layer 0 vs layers 1-11
    layer0_params = jax.tree.map(lambda p: p[0], params['layers'])
    rest_params = jax.tree.map(lambda p: p[1:], params['layers'])

    # --- Layer 0: no value residual (v1 doesn't exist yet) ---
    @jax.checkpoint
    def block_first(x, layer_params):
        x_norm = rms_norm(x)
        ...
        x = x + ffwd
        return x.astype(compute_dtype), v  # v is this layer's values → becomes v1

    x, v1 = block_first(x, layer0_params)

    # --- Layers 1-11: blend v1 into each layer's V via learned lamb ---
    @jax.checkpoint
    def block_fn(x, layer_params):
        ...
        k = k.reshape(B, T, num_heads, head_dim)
        v = v.reshape(B, T, num_heads, head_dim)
        # Value residual: mix current V with layer 0's V
    ...
    x, _ = jax.lax.scan(block_fn, x, rest_params)

    x = rms_norm(x)

    # CHANGE 5b: separate lm_head
...
def muon_apply(grad_layers, muon_buffers, step):
    ...
    def update_one(g, buf):
        ...
        if g.ndim >= 2:
            ...
            # Simple momentum SGD for scalars/vectors (like lamb)
