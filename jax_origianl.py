...
weight_decay = 0.1
n_embd = 768
num_heads = 12                                      # FIX: was 6, PyTorch uses 12
...
def init_model_params(key, vocab_size, n_embd):
    # We need: 1 (token_emb) + n_layer * 5 (W_q, W_k, W_v, W_out, W_ffwd, W_ffwd_project)
    # W_ffwd_project uses separate keys too
    ...
    params = {
        ...
        'layers': {
            # Standard (n_embd, n_embd) projections matching PyTorch's nn.Linear
...
def rotary(seq_len):
    """Precompute cos/sin tables for RoPE. Base 10000 to match PyTorch."""
    inv_freq = 1.0 / (10000.0 ** (jnp.arange(0, head_dim, 2).astype(jnp.float32) / head_dim))
...
def forward(params, idx):
    ...
    # Token embedding (no initial rmsnorm — PyTorch doesn't have one)
    ...
    def block_fn(x, layer_params):
        ...
        ffwd = jax.nn.gelu(ffwd, approximate=False)
